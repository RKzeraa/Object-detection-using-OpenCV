...
for i in range(len(imgRGB)): #Utilizando diversas imagens no algoritimo de Detecção
    ...
    sobelx = cv2.Sobel(gauss,cv2.CV_64F,1,0,ksize=11)
    sobely = cv2.Sobel(gauss,cv2.CV_64F,0,1,ksize=11)
    gradmag = np.sqrt(sobelx**2 + sobely**2)
    cv2.imshow("sobelX", sobelx)
    cv2.waitKey(0)
    cv2.imshow("sobelY", sobely)
    cv2.waitKey(0)
    cv2.imshow("Magnitude", gradmag)
    cv2.waitKey(0)
    ...
    # Testar a transformada de Hough, e segmentação
